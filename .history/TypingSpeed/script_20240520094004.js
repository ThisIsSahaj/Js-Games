...
{
  try {
    const apiFetch = await fetch("https://typeracingapi.rishikc.com");
    console.log('apiFetch status: ', apiFetch.status);
    
    if (!apiFetch.ok) {
        throw new Error(`HTTP error! Status: ${apiFetch.status}`);
    }
